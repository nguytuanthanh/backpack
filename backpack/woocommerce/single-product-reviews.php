...
<div id="reviews" class="woocommerce-Reviews">
	<div id="comments">
		<h4 class="slide-title"><?php
			if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' && ( $count = $product->get_review_count() ) ) {
				/* translators: 1: reviews count 2: product name */
				printf( esc_html( _n( '%1$s nhận xét cho %2$s', '%1$s nhận xét cho %2$s', $count, 'woocommerce' ) ), esc_html( $count ), '<span>' . get_the_title() . '</span>' );
			} else {
				_e( 'Nhận xét', 'woocommerce' );
			}
		?></h4>
		...
		<?php else : ?>
			...
			<p class="woocommerce-noreviews"><?php _e( 'Chưa có nhận xét cho sản phẩm này.', 'woocommerce' ); ?></p>
